...
void poly_t1_unpack(Poly *r, uint8_t a[]) {
    for(int i = 0; i < N/4; i++) {
		r->coeffs[4*i+0] = (int32_t)(((uint32_t)(a[5*i+0]>>0) | ((uint32_t)(a[5*i+1]) << 8)) & 0x3FF);
		r->coeffs[4*i+1] = (int32_t)(((uint32_t)(a[5*i+1]>>2) | ((uint32_t)(a[5*i+2]) << 6)) & 0x3FF);
		r->coeffs[4*i+2] = (int32_t)(((uint32_t)(a[5*i+2]>>4) | ((uint32_t)(a[5*i+3]) << 4)) & 0x3FF);
		r->coeffs[4*i+3] = (int32_t)(((uint32_t)(a[5*i+3]>>6) | ((uint32_t)(a[5*i+4]) << 2)) & 0x3FF);
	}
}
